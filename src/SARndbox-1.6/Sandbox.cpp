...
Sandbox::WaterToolFactory* Sandbox::WaterTool::initClass(Vrui::ToolManager& toolManager)
	...
	factory=new WaterToolFactory("WaterTool","Gerenciamento de agua",0,toolManager);
...
Sandbox::LocalWaterToolFactory* Sandbox::LocalWaterTool::initClass(Vrui::ToolManager& toolManager)
	{
	/* Create the tool factory: */
	factory=new LocalWaterToolFactory("LocalWaterTool","Adicionar agua localmente",0,toolManager);
...
void Sandbox::LocalWaterTool::addWater(GLContextData& contextData) const
	...
	if(1)
		...
		Vrui::Point rainPos=Vrui::getInverseNavigationTransformation().transform(getButtonDevicePosition(0));
		//////////////////////////////////////////////////////////////////////
		//				Faz tamnanho da nascente							//
		//////////////////////////////////////////////////////////////////////
		Vrui::Scalar rainRadius=Vrui::getPointPickDistance()*Vrui::Scalar(0.5);
		...
		//Faz cada um dos pontos de agua
		for (int k = 0; k < Spontos.size(); k++) {
			glBegin(GL_POLYGON);
			for (int i = 0; i < 32; ++i) {

				//Vrui::Scalar angle = Vrui::Scalar(5) * Math::Constants<Vrui::Scalar>::pi * Vrui::Scalar(i) / Vrui::Scalar(32);
				//glVertex(Spontos[0] + x * Math::cos(angle) + y * Math::sin(angle));
				Vrui::Scalar angle = Vrui::Scalar(2)* Math::Constants<Vrui::Scalar>::pi * Vrui::Scalar(i)/ Vrui::Scalar(28);
				glVertex(Spontos[k] + x * Math::cos(angle)+ y * Math::sin(angle));
			}
			glEnd();
		}
...
void Sandbox::addWater(GLContextData& contextData) const {
	...
	if (!rainObjects.getLockedValue().empty()) {
		...
		Vrui::Scalar rainRadius=Vrui::getPointPickDistance()*Vrui::Scalar(sizeSource);
		...
		Vrui::Vector x = Geometry::normal(z);
		Vrui::Vector y = Geometry::cross(z, x);
		//Essas duas linhas aumentam o tamanho das nascentes, mas parece não fazer nada quando usa os métodos .normalize().
		//x*=rainRadius/Geometry::mag(x);
		//y*=rainRadius/Geometry::mag(y);
		//////////////////////////////////////////////////////////////
		//					Modificado								//
		...
		glPopAttrib();
			...
			secar = -0.25;
			if (Spontos.size()) {
				glVertexAttrib1fARB(1, secar / waterSpeed);
				glBegin(GL_POLYGON);
				for (int i = 0; i < 32; ++i) {
					Vrui::Scalar angle = Vrui::Scalar(5) * Math::Constants<Vrui::Scalar>::pi * Vrui::Scalar(i) / Vrui::Scalar(32);
					glVertex(Spontos[0] + x * Math::cos(angle) + y * Math::sin(angle));
			...
			glVertexAttrib1fARB(1, rainStrength / waterSpeed);
			for (int k = 1; k < Spontos.size(); k++) {// Começa no segundo elemento
				glBegin(GL_POLYGON);
				for (int i = 0; i < 32; ++i) {
					Vrui::Scalar angle = Vrui::Scalar(2) * Math::Constants<Vrui::Scalar>::pi * Vrui::Scalar(i) / Vrui::Scalar(5);
					glVertex(Spontos[k] + x * Math::cos(angle) + y * Math::sin(angle));
				}
				glEnd();
			}
...
//					Modificado								//
//////////////////////////////////////////////////////////////
void Sandbox::onWaterSourceCallback(GLMotif::ToggleButton::ValueChangedCallbackData* cbData)
	{
	onWaterSource=cbData->set;
	std::cout << onWaterSource << std::endl;
	}
...
//					Modificado								//
//////////////////////////////////////////////////////////////
void Sandbox::showWaterSourceControlDialogCallback(Misc::CallbackData* cbData)
	{
	Vrui::popupPrimaryWidget(waterSourceControlDialog);
	}
...
void Sandbox::waterSourceSizeSliderCallback(GLMotif::TextFieldSlider::ValueChangedCallbackData* cbData)
	{
	sizeSource=cbData->value;
	}
...
GLMotif::PopupMenu* Sandbox::createMainMenu(void)
	...
	mainMenuPopup->setTitle("Caixa e Agua");
	...
	//						Modificado			       		    //
	//////////////////////////////////////////////////////////////
	onWaterSourceToggle=new GLMotif::ToggleButton("onWaterSourceToggle",mainMenu,"Ativa Nascente");
	onWaterSourceToggle->setToggle(false);
	onWaterSourceToggle->getValueChangedCallbacks().add(this,&Sandbox::onWaterSourceCallback);
	...
	/* Create a button to show the water source control dialog: */
	GLMotif::Button* showWaterSourceControlDialogButton=new GLMotif::Button("showWaterSourceControlDialogButton",mainMenu,"Show Water Source Simulation Control");
	showWaterSourceControlDialogButton->getSelectCallbacks().add(this,&Sandbox::showWaterSourceControlDialogCallback);
...
GLMotif::PopupWindow* Sandbox::createWaterSourceControlDialog(void)
	{
	const GLMotif::StyleSheet& ss=*Vrui::getWidgetManager()->getStyleSheet();

	/* Create a popup window shell: */
	GLMotif::PopupWindow* waterControlDialogPopup=new GLMotif::PopupWindow("WaterControlDialogPopup",Vrui::getWidgetManager(),"Propriedades da Fonte");
	...
	GLMotif::RowColumn* waterSourceControlDialog=new GLMotif::RowColumn("WaterControlDialogPopup",waterControlDialogPopup,false);
	waterSourceControlDialog->setOrientation(GLMotif::RowColumn::VERTICAL);
	waterSourceControlDialog->setPacking(GLMotif::RowColumn::PACK_TIGHT);
	waterSourceControlDialog->setNumMinorWidgets(2);

	new GLMotif::Label("WaterSpeedLabel",waterSourceControlDialog,"Tamanho");

	waterSourceSizeSlider=new GLMotif::TextFieldSlider("WaterSpeedSlider",waterSourceControlDialog,8,ss.fontHeight*10.0f);
	waterSourceSizeSlider->getTextField()->setFieldWidth(7);
	waterSourceSizeSlider->getTextField()->setPrecision(2);
	waterSourceSizeSlider->getTextField()->setFloatFormat(GLMotif::TextField::SMART);
	waterSourceSizeSlider->setSliderMapping(GLMotif::TextFieldSlider::EXP10);
	waterSourceSizeSlider->setValueRange(0.1,4.0,0.05);
	//Adiciona a marcação numa posição da barra.
	//waterSourceSizeSlider->getSlider()->addNotch(0.0f);
	waterSourceSizeSlider->setValue(sizeSource);
	waterSourceSizeSlider->getValueChangedCallbacks().add(this,&Sandbox::waterSourceSizeSliderCallback);

	new GLMotif::Label("WaterSourceTamanhoLabel",waterSourceControlDialog,"Tamanho");

	waterMaxStepsSlider=new GLMotif::TextFieldSlider("WaterMaxStepsSlider",waterSourceControlDialog,8,ss.fontHeight*10.0f);
	...
	waterMaxStepsSlider->setValueRange(0,64,1);
	...
	waterSourceControlDialog->manageChild();
...
Sandbox::Sandbox(int& argc,char**& argv)
	:Vrui::Application(argc,argv),
	 camera(0),
	 frameFilter(0),pauseUpdates(false), onWaterSource(false) /*FURB*/,
	 ...
	 waterRenderer(0),
	 sun(0),
	 mainMenu(0),pauseUpdatesToggle(0),waterSourceControlDialog(0),waterControlDialog(0),
	 waterSpeedSlider(0),waterMaxStepsSlider(0),frameRateTextField(0),waterAttenuationSlider(0),
	 controlPipeFd(-1), onWaterSourceToggle(0)/*FURB*/
	{
	...
	//					Modificado								//
	//////////////////////////////////////////////////////////////
	addEventTool("Adiciona Nascentes",0,1);
	addEventTool("Ativa Nascentes",0,2);
	...
	//					Modificado								//
	//////////////////////////////////////////////////////////////
	/* na função rawDepthFrameDispatcher foi adicionado trechos de
	 * código que eram originais da função depthStreamingCallback
	 * que fazia parte do CalibrateProjector */
	/* Start treaming depth frames: */
	...
	if(waterTable!=0){
		...
		waterSourceControlDialog = createWaterSourceControlDialog();
	}
...
Sandbox::~Sandbox(void)
	...
	delete mainMenu;
	delete waterSourceControlDialog;
	delete waterControlDialog;
	std::cout << "Destruiu" << std::endl;
	close(controlPipeFd);
	}

/* Não é mais utilizado,  o código dessa função foi copiado para a função
 * rawDepthFrameDispatcher, pois é chamada no callback da camera de  profundidade*/
void Sandbox::depthStreamingCallback(const Kinect::FrameBuffer& frameBuffer)
	{
	/* Do nothing if currently capturing background frames: */
...
void Sandbox::startBackgroundCapture(void)
	{
	std::cout<< "startBackgroundCapture" << capturingBackground << std::endl;
	...
	std::cout<<"startBackgroundCapture: Capturing "<<numBackgroundFrames<<" background frames..."<<std::endl;
	camera->captureBackground(numBackgroundFrames,true,Misc::createFunctionCall(this,&Sandbox::backgroundCaptureCompleteCallback));
	}

void Sandbox::backgroundCaptureCompleteCallback(Kinect::Camera&)
	{

	std::cout<< "backgroundCaptureCompleteCallback" << std::endl;
...
void Sandbox::startTiePointCapture(void)
	{
	std::cout<< "startTiePointCapture" << std::endl;
	/* Bail out if already capturing a tie point or background: */
	if(capturingBackground||capturingTiePoint)
		return;
			...
			for(std::vector<DepthCentroidBlob>::iterator bIt=blobs.begin();bIt!=blobs.end();++bIt)
				...
				if(currentBlob!=0)
					...
					--numCaptureFrames;
					 ...
					 std::cout<<" Capture frame"<<numCaptureFrames  <<std::endl;
					...
					std::cout<<" done"<<std::endl;
					//std::cout<<"Tie point: "<<tp.o[0]<<", "<<tp.o[1]<<", "<<tp.o[2]<<std::endl;
...
//					Modificado								//
//////////////////////////////////////////////////////////////
void Sandbox::LocalWaterTool::registerPoint()
{
	Vrui::Point newRain = Vrui::getInverseNavigationTransformation().transform(getButtonDevicePosition(0));
	//Adciona o primeiro ponto.
	if (Spontos.size() == 0)
		Spontos.push_back(newRain);
	else {
		if (Spontos[Spontos.size() - 1] != newRain) {
			Spontos.push_back(newRain);
		}
	}
...
void Sandbox::LocalWaterTool::activeWaterSources()
{
	...
	if (adding != 0.0f)
	{
		...
		//Faz cada um dos pontos de água
		for (int k = 0; k < Spontos.size(); k++) {
			glBegin(GL_POLYGON);
			for (int i = 0; i < 32; ++i) {
				Vrui::Scalar angle = Vrui::Scalar(2)* Math::Constants<Vrui::Scalar>::pi * Vrui::Scalar(i)
						/ Vrui::Scalar(32);
				glVertex(Spontos[k] + x * Math::cos(angle) + y * Math::sin(angle));
			}
			glEnd();
...
{
	if (cbData->newButtonState) {
		Vrui::Point ponto = Vrui::getInverseNavigationTransformation().transform(cbData->inputDevice->getPosition());
		switch (eventId) {
		...
		case 1:
			//Adciona o primeiro ponto.
			if (Spontos.size() == 0) {
				Spontos.push_back(ponto);
				Spontos.push_back(Vrui::Point(0.0f, 0.0f, 0.0f));
				Vrui::Point(5.0f, 6.0f, 7.0f);
			} else if (Spontos[Spontos.size() - 1] != ponto) {
				Spontos.push_back(ponto);
			}
		...
		case 2:
			//this->LocalWaterTool::activeWaterSources();
